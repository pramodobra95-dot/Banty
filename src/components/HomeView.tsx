...
export default function HomeView({
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-12">
      ...
      {/* 4. DYNAMIC PRODUCTS SECTIONS */}
      <div id="featured-products-catalog" className="max-w-7xl mx-auto px-6 py-8 space-y-12">
        ...
        <div className="space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-yellow-500 font-bold text-xs uppercase tracking-wider block">Handpicked Solutions</span>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.length === 0 ? (
              [...Array(3)].map((_, idx) => (
                <div key={idx} className="bg-white rounded-xl border border-slate-100 overflow-hidden shadow-xs animate-pulse flex flex-col h-[400px]">
              ...
              [...filteredProducts]
                ...
                .slice(0, 6)
                .map((p) => (
                  <div key={`featured-${p.id}`} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs hover:shadow-lg transition-all flex flex-col group relative">
                    ...
                    <div onClick={() => handleOpenProduct(p)} className="h-44 bg-slate-100 overflow-hidden relative cursor-pointer">
                    ...
                    <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
                      <div>
                        <p className="text-[10px] text-[#0066FF] font-bold uppercase tracking-wider">By: {p.vendorName}</p>
        ...
        {/* B. NEWEST PRODUCTS SECTION */}
        <div className="space-y-6">
          <div className="border-b border-slate-200 pb-4">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-blue-600 font-bold text-xs uppercase tracking-wider block">Fresh Additions</span>
                <h3 className="text-lg md:text-xl font-black text-slate-900 mt-1">
                  Newest Products
                </h3>
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {products.length === 0 ? (
              ...
              [...filteredProducts]
                ...
                .slice(0, 6)
                .map((p) => (
                  <div key={`newest-${p.id}`} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-xs hover:shadow-lg transition-all flex flex-col group relative">
                    <div onClick={() => handleOpenProduct(p)} className="h-44 bg-slate-100 overflow-hidden relative cursor-pointer">
                      <img src={p.images[0] || "https://images.unsplash.com/photo-1531403009284-440f080d1e12?w=500&auto=format&fit=crop"} alt={p.name} className="w-full h-full object-cover" />
                      <span className="absolute bottom-3 right-3 bg-slate-900/80 backdrop-blur-xs text-white text-[10px] px-2 py-1 rounded">{p.category}</span>
                    </div>
